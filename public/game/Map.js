...
class Map {
  ...
  generatePorts(board) {
    ...
    for (var town = 0; town < board.hw_towns.length - 1; town++) {
      var path = this.findPath(board.hw_towns[town], board.hw_towns[town + 1], ["town"], true);
      if (path == null || path.length > portNum) {
        path = this.findPath(board.hw_towns[town], board.hw_towns[town + 1], ["town"], false);
        pn++;
      }
  ...
  calcAIHelpers(board) {
    for (var partyIndex = 0; partyIndex < board.hw_parties_count; partyIndex++) {
      for (var x = 0; x < board.hw_xmax; x++) {
        for (var y = 0; y < board.hw_ymax; y++) {
          var field = this.getField(x, y, board);
          field.profitability[partyIndex] = -this.findPath(field, board.hw_parties_capitals[partyIndex], [], true).length;
          var neighbours = this.getFurtherNeighbours(field);
  ...
  calcArmiesProfitability(party, board) {
    ...
    function findBestMoveVal(army) {
      var moves = self.getPossibleMoves(army.field, true, false);
  ...
  calcNeighboursInfo(party, field) {
    ...
    var furtherNeighbours = this.getFurtherNeighbours(field);
    ...
    function findBestMoveVal(army) {
      ...
      for (var i = 0; i < moves.length; i++) {
        if (moves[i] != field && (!moves[i].army || moves[i].army.party < 0 || moves[i].army.party == party)) {
          moves[i].tmp_prof = -self.getDistance(moves[i], field);
